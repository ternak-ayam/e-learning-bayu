    ...
    <div class="md:px-52 py-10 ">
        <div class="px-4">
            ...
            <h5 class="mb-2 ml-2 md:text-start text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
               {{$pertemuan ? 'Edit Absensi' : 'Tambah Absensi'}}</h5>
            <div
                ...
                <div class="w-full p-4">
                    ...
                    <form method="POST" action="{{$pertemuan ? route('admin.update.absensi', $pertemuan->id) : route('admin.add.absensi') }}">
                      @if($pertemuan)
                        @method('PUT')
                      @endif
                        @csrf
                        <div class="mb-6">
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Judul
                                Absensi</label>
                            <input type="text" id="email" name="judul" value="{{$pertemuan ? $pertemuan->judul : ''}}"
